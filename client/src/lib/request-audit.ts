...
export const requestAudit = {
   ...
   * Log a request start
   */
  logRequestStart(endpoint: string, method: string) {
   ...
   */
  logRequestComplete(endpoint: string, method: string, status: number, duration: number) {
    const record = requestLog.find(
      (r) => r.endpoint === endpoint && r.method === method && !r.completed
    );
    if (record) {
   ...
   */
  checkForLoops(requestId: string, currentTime: number) {
    const windowStart = currentTime - TIME_WINDOW_MS;
    const recentRequests = requestLog.filter(
      (r) => `${r.method} ${r.endpoint}` === requestId && r.timestamp > windowStart
    );

    if (recentRequests.length > LOOP_THRESHOLD) {
      console.error(
        `🚨 [REQUEST AUDIT] INFINITE LOOP DETECTED!`,
        `Request: ${requestId}`,
        `Count: ${recentRequests.length} in last ${TIME_WINDOW_MS}ms`,
        `Recent requests:`,
   ...
   */
  alertInfiniteLoop(requestId: string, count: number) {
    // You can dispatch this to error tracking service
    console.error(
      `⚠️  Infinite request loop detected: ${requestId} (${count} requests)`
    );
    
    // Optionally pause the app or redirect to error page
    if (typeof window !== 'undefined') {
      window.dispatchEvent(
        new CustomEvent('apiLoopDetected', {
          detail: { requestId, count },
        })
      );
    }
  },
...
if (typeof window !== 'undefined') {
  (window as any as Window & { requestAudit: typeof requestAudit }).requestAudit = requestAudit;
}
