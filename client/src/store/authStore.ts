...
interface AuthState {
  ...
  otpName: string | null;
  otpSent: boolean;
  // Temporary registration state (in-memory only)
  registerName: string | null;
  registerEmail: string | null;
  registerPassword: string | null;
  registerOtpSent: boolean;
  // Rehydration tracking - true when localStorage data has been loaded
  _isHydrated: boolean;
...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      login: async (email: string, password: string) => {
        ...
        try {
          ...
          set({
            ...
            isLoading: false,
            loginMethod: 'password', // STRICT: Track password login
            otpMode: null, // STRICT: Clear OTP state
            otpSent: false,
            otpEmail: null,
            otpName: null,
          });
      ...
      sendOTP: async (email: string, mode: 'register' | 'login' | 'reset_password') => {
        ...
        try {
          ...
          if (response.success) {
            ...
            set({
              loginMethod: mode === 'login' ? 'otp' : null, // Track loginMethod for login mode
              otpMode: mode,
              otpSent: true,
              otpEmail: email,
              isLoading: false,
      ...
      setLoginMethod: (method: 'password' | 'otp') => {
