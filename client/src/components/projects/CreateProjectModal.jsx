...
export function CreateProjectModal({
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {children || <Button className="gap-2">
            <Plus className="w-4 h-4" />
            New Project
          </Button>}
      </DialogTrigger>
      <AnimatePresence>
        {open && <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto">
            <motion.div initial={{
          opacity: 0,
          scale: 0.95
        }} animate={{
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 mt-4">
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input id="name" placeholder="e.g., Customer Support Bot" {...register('name')} className={errors.name ? 'border-destructive' : ''} maxLength={100} />
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea id="description" placeholder="What does this agent do? What's its purpose?" rows={2} {...register('description')} maxLength={500} />
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={model} onValueChange={value => setValue('model', value)}>
                      ...
                      <SelectContent>
                        {models.map(m => <SelectItem key={m.value} value={m.value} disabled={!m.available} className={!m.available ? "opacity-50 cursor-not-allowed" : ""}>
                            <div className="flex flex-col">
                              <span className="flex items-center gap-2">
                                {m.label}
                                {!m.available && <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded border uppercase tracking-wider">
                                    Unavailable
                                  </span>}
                              </span>
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea id="systemPrompt" placeholder="Define your AI agent's behavior, personality, and response style. Example: 'You are a helpful customer support assistant. Be friendly, professional, and always aim to resolve issues efficiently.'" rows={4} {...register('systemPrompt')} className={errors.systemPrompt ? 'border-destructive' : ''} maxLength={4000} />
