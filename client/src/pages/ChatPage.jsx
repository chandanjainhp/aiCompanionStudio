...
export default function ChatPage() {
  ...
  return (
    <TooltipProvider>
      <div className="dark h-screen flex overflow-hidden bg-[#0B0F1A] relative">
        {/* Mobile Overlay */}
        <div
          className={cn(
        ...
        <aside
          className={cn(
            "h-screen flex-col bg-[#0E1324] border-r border-white/10 flex transition-all duration-300 ease-in-out",
            "fixed inset-y-0 left-0 w-[280px] z-50",
            "md:relative md:z-30",
            !projectSidebarOpen
              ? "-translate-x-full md:w-0 md:opacity-0 md:translate-x-0"
              : sidebarCollapsed
                ? "translate-x-0 md:w-[72px]"
          ...
          <div className="h-16 flex items-center px-4 border-b border-white/10 shrink-0">
            {!sidebarCollapsed || window.innerWidth < 768 ? (
              <div className="flex items-center gap-3 w-full">
                ...
                <span className="font-bold text-white truncate text-sm tracking-tight">
          ...
          <div className="p-3">
            <Button
              onClick={() => {
                ...
                if (window.innerWidth < 768) toggleProjectSidebar();
              }}
              variant="outline"
              className={cn(
                "w-full bg-white/5 border-white/10 hover:bg-white/10 text-white transition-all",
                sidebarCollapsed && window.innerWidth >= 768
                  ? "p-0 h-10 w-10 mx-auto justify-center"
              ...
              <div key={label} className={cn(items.length === 0 && "hidden")}>
                ...
                <div className="space-y-0.5">
                  {items.map(conv => {
                    ...
                    const content = (
                      <div
                        ...
                        className={cn(
                          ...
                          sidebarCollapsed && window.innerWidth >= 768 && "justify-center px-0 h-10 w-10 mx-auto"
                        )}
                      >
                    ...
                    return sidebarCollapsed && window.innerWidth >= 768 ? (
                      <Tooltip key={conv.id} delayDuration={0}>
                        <TooltipTrigger asChild>{content}</TooltipTrigger>
                        <TooltipContent
                          side="right"
                          className="bg-[#161B22] border-white/10 text-[11px] font-medium text-white shadow-xl"
                        >
                          {typeof conv?.title === 'string' ? conv.title : "New Chat"}
                        </TooltipContent>
                      </Tooltip>
