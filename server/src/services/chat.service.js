...
import { getFormattedCoreMemory } from './memory.service.js';

/**
 * Get or create system prompt for project (with core memory injection)
 * Returns system prompt + injected core memory facts
 */
export const getActiveSystemPrompt = async (projectId) => {
  ...
  // 2. Fallback to Project's default systemPrompt and fetch coreMemory
  if (!basePrompt) {
    const project = await prisma.project.findUnique({
      where: { id: projectId },
      select: { systemPrompt: true, coreMemory: true },
    });

    basePrompt = project?.systemPrompt || 'You are a helpful AI assistant.';

    // Inject core memory if it exists
    if (project?.coreMemory) {
      return basePrompt + getFormattedCoreMemory(project.coreMemory);
    }

    return basePrompt;
  }

  // For versioned prompts, also fetch and inject core memory
  const project = await prisma.project.findUnique({
    where: { id: projectId },
...
export const sendChatMessage = async (projectId, conversationId, userMessage, userId) => {
  ...
  try {
    ...
    console.log('🧠 [sendChatMessage] Triggering async memory extraction...');
    (async () => {
      try {
        const { updateCoreMemoryFromConversation } = await import('./memory.service.js');

        // Get current core memory for memory service
        const projectWithMemory = await prisma.project.findUnique({
          where: { id: projectId },
          select: { coreMemory: true },
        });

        const memoryUpdate = await updateCoreMemoryFromConversation(
          projectId,
          userMessage,
          savedAssistantMessage.content,
          projectWithMemory?.coreMemory || ''
        );

        if (memoryUpdate) {
          console.log('✅ [sendChatMessage] Memory extraction completed:', {
            newFacts: memoryUpdate.newFacts.length,
            timestamp: memoryUpdate.updatedAt,
          });
        }
      } catch (memoryError) {
        console.warn('⚠️  [sendChatMessage] Memory extraction error (non-blocking):', memoryError.message);
      }
    })();
