...
export const createProject = asyncHandler(async (req, res) => {
  const userId = req.user.userId;
  const { name, description, model, temperature, maxTokens, systemPrompt, coreMemory } = req.body;
...
/**
 * Get project's core memory
 * GET /api/v1/projects/:id/memory
 */
export const getCoreMemory = asyncHandler(async (req, res) => {
  const userId = req.user.userId;
  const { id } = req.params;
  ...
  res.status(200).json({
    ...
    data: {
      ...
      memoryUpdatedAt: project.memoryUpdatedAt,
    },
  });
});

/**
 * Update project's core memory (manual update)
 * PUT /api/v1/projects/:id/memory
 */
export const updateCoreMemory = asyncHandler(async (req, res) => {
  const userId = req.user.userId;
  const { id } = req.params;
  const { coreMemory } = req.body;

  if (typeof coreMemory !== 'string') {
    return res.status(400).json({
      success: false,
      message: 'coreMemory must be a string',
    });
  }
  ...
  res.status(200).json({
    ...
    data: {
      ...
      coreMemory: updatedProject.coreMemory,
      memoryUpdatedAt: updatedProject.memoryUpdatedAt,
    },
  });
});

/**
 * Clear project's core memory
 * DELETE /api/v1/projects/:id/memory
 */
export const clearCoreMemory = asyncHandler(async (req, res) => {
  const userId = req.user.userId;
  const { id } = req.params;
  ...
  res.status(200).json({
    success: true,
    message: 'Core memory cleared successfully',
    data: {
      projectId: updatedProject.id,
      coreMemory: '',
      memoryUpdatedAt: updatedProject.memoryUpdatedAt,
