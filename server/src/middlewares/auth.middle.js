...
let verifyJWTCallCount = 0;
const VERIFY_JWT_LIMIT = 10;
...
export const verifyJWT = asyncHandler(async (req, res, next) => {
  ...
  try {
    console.log(`🔐 [verifyJWT] CALL #${verifyJWTCallCount} for ${req.method} ${req.originalUrl}`);
    
    if (verifyJWTCallCount > VERIFY_JWT_LIMIT) {
      console.error(`❌ [verifyJWT] INFINITE LOOP DETECTED! ${verifyJWTCallCount} calls`);
    }
    ...
    console.log(`✅ [verifyJWT] CALL #${verifyJWTCallCount} SUCCESS for user: ${user.email}`);
